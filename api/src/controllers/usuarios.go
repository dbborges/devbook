...
func BuscarUsuario(w http.ResponseWriter, r *http.Request) {
	...
	repositorio := repositorios.NovoRepositorioDeUsuarios(db)
	usuario, erro := repositorio.BuscarPorID(uint64(usuarioID))
	if erro != nil {
		respostas.Erro(w, http.StatusInternalServerError, erro)
		return
	}
...
func DeletarUsuario(w http.ResponseWriter, r *http.Request) {
	...
	if usuarioID != usuarioIDNoToken {
		respostas.Erro(w, http.StatusForbidden, errors.New("não é possível delete um usuário que não seja o seu"))
		return
	}
...
func AtualizarSenha(w http.ResponseWriter, r *http.Request) {
	usuaioIDNoToken, erro := autenticacao.ExtrairUsuarioID(r)
	if erro != nil {
		respostas.Erro(w, http.StatusUnauthorized, erro)
		return
	}
	...
	fmt.Println("ATUAL: " + senha.Atual)
	fmt.Println(senhaSalvaNoBanco)
